...
# # ------------------ instance ------------------
# def phase_retrieval(n=200, m=5000, seed=123, normalize_rows=True, xstar_norm=1.0, outlier_ratio=0.0):
...
#         # Add large magnitude noise to test L1-norm robustness
...
# # ------------------ Subgradient Algotithm via Scaled Polyak Step-size  ------------------
# def SG_SP(
#     x0, A, b, x_star, f_star, sigma,
#     max_iter=4000,
#     tol=1e-12,
# ):

#     x = x0.copy()
#     f, g = f_and_grad(x, A, b)
...
#         end = len(err) - 1
#     else:
#         end = int(idx[0])  # keep ratios up to end-1 (since ratios have length-1)

#     # end is an index into err; ratios are defined for k=0..len-2
#     max_len = max(1, min(end, len(err) - 1))
#     return max_len


# # ------------------ Phase Transition Study ------------------
# def run_phase_transition():
#     print("Starting Phase Transition Study... (This may take a few minutes)")
#     n_val = 1000  
...
#             A, b, x_star = phase_retrieval(n=n_val, m=m_val, normalize_rows=True)
           
#             x0 = np.random.standard_normal(n_val)
            
#             hist = SG_SP(x0, A, b, x_star, f_star=0, sigma=1, max_iter=500)
            
#             relative_error = hist["err"][-1] / np.linalg.norm(x_star)
...
#     #plt.axvline(x=4, color='r', linestyle='--', label="Theoretical Threshold (m≈4n)")
#     plt.xlabel(r"$m/n$")
...
#     # Generate a fixed random initial point for a fair comparison
...
#         A, b, x_star = phase_retrieval(n=n, m=m, seed=123, normalize_rows=True)
#         f_star = 0
...
#         # Calculate ratios and clipping lengths
#         rf = safe_ratio(hist["f"])
#         re = safe_ratio(hist["err"])
#         L = clip_ratios_to_err(hist, err_floor=1e-14)
...
#                      linewidth=2, label=rf"$\alpha = {alpha}$")
#     plt.xlabel("Iteration")
#     plt.ylabel(r"$e_k=\min(\|x_k-x^\star\|,\|x_k+x^\star\|)$")
#     plt.grid(True, which="both", linestyle='--', alpha=0.5)
#     plt.legend()

#     # ---- Figure 3: empirical linear ratios (f) ----
#     plt.figure(figsize=(8, 6))
...
#         plt.plot(it, results[alpha]["rf"], linestyle=styles[i], marker=markers[i], 
#                  color=colors[i], markevery=marker_every, linewidth=1.6, 
#                  label=rf"$\alpha = {alpha}$")
        
#     plt.xlabel("Iteration")
#     plt.ylabel(r"$f_{k+1}/f_k$")
#     plt.grid(True, linestyle='--', alpha=0.5)
#     plt.legend()

#     # ---- Figure 4: empirical linear ratios (err) ----
#     plt.figure(figsize=(8, 6))
#     for i, alpha in enumerate(alphas_2):
#         L = results[alpha]["L"]
#         it = np.arange(L)
...
#         plt.plot(it, results[alpha]["re"], linestyle=styles[i], marker=markers[i], 
#                  color=colors[i], markevery=marker_every, linewidth=1.6, 
#                  label=rf"$\alpha = {alpha}$")
